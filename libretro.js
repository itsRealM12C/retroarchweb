...
function mountBrowserFS() {
   ...
   Module.FS.writeFile("/home/root/retroarch/cores/" + currentCore + "_libretro.core", new Uint8Array());
   for (let core of Object.keys(libretroCores)) {
      Module.FS.writeFile("/home/root/retroarch/cores/" + core + "_libretro.core", new Uint8Array());
   }
}
...
function finishFileSystemSetup() {
   ...
   mfs.mount('/home/root/retroarch', zipfs);
   mfs.mount('/home/root/retroarch/cores', new BrowserFS.FileSystem.InMemory());
   mfs.mount('/home/root/retroarch/userdata', afs);
   mfs.mount('/home/root/retroarch/userdata/content/downloads', xhrfs);
...
function uploadData(data, name) {
   ...
   Module.FS.writeFile('/home/root/retroarch/userdata/content/' + name, data, {
      encoding: 'binary'
   });
...
$(function() {
   ...
   $('#core-selector a').click(function(e) {
      ...
      if (Module && retroArchRunning) {
         Module.retroArchSend("LOAD_CORE /home/root/retroarch/cores/" + core + "_libretro.core");
...
async function loadCore(core, args) {
   ...
   ModuleBase.arguments = args || ["-v", "--menu", "-c", "/home/root/retroarch/userdata/retroarch.cfg"];
   ModuleBase.preRun = [modulePreRun];
   ModuleBase.canvas = canvas;
   ModuleBase.corePath = "/home/root/retroarch/cores/" + core + "_libretro.core";
...
async function relaunch(core, content) {
   ...
   // parse core name from full path ("/home/root/retroarch/cores/NAME_libretro.core")
   currentCore = core.slice(0, -14).split("/").slice(-1)[0];

   localStorage.setItem("core", currentCore);
   await loadCore(currentCore, ["-v", content, "-c", "/home/root/retroarch/userdata/retroarch.cfg"]);
   mountBrowserFS();
   Module.callMain(Module.arguments);
}
